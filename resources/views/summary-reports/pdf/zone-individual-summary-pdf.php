...
<body>
<?php if(isset($get_zone_data)&& count($get_zone_data)>0){

    foreach ($get_zone_data as $key => $zone_data) {

        $sales_person_summary = $zone_data['sales_person_summary'];
        $sales_person_transaction = $zone_data['sales_person_transaction'];
        $last_card_report = $zone_data['last_card_report'];
        $observation_status = $zone_data['observation_status'];

        ?>
        <?php if (isset($sales_person_summary)) { ?>
            <div class="box-body">

                <section class="pg" <?php echo ($key!=0) ? 'style="page-break-before:always;"':''?>>
                    <div class="top-header">
                        <p style="text-align: center; font-size: 13px;"> Southeast Bank Ltd. <br/> Card Division, Head
                            Office, Dhaka
                            <br/> <?php echo isset($sales_person_summary->report_zone_name) ? $sales_person_summary->report_zone_name : ''; ?>
                        </p>
                    </div>
                    <p> Commission for the Month
                        of: <?php $month = str_pad($sales_person_summary->report_month, 2, '0', STR_PAD_LEFT);
                        $report_date = "$sales_person_summary->report_year-$month-01";
                        echo date('F-Y', strtotime($report_date)); //echo date('F-Y');?> </p>
                    <p> Name of the Sales
                        Executive: <?php echo isset($sales_person_summary->report_ExecutiveName) ? $sales_person_summary->report_ExecutiveName : ''; ?> </p>
                    <p> Date of
                        Joining: <?php echo isset($sales_person_summary->report_dateOfjoining) ? $sales_person_summary->report_dateOfjoining : ''; ?> </p>
                    <p>
                        Designation: <?php echo isset($sales_person_summary->report_DesigTitle) ? $sales_person_summary->report_DesigTitle : ''; ?>
                        (Minimum
                        Target: <?php echo isset($sales_person_summary->report_sales_persons_target) ? $sales_person_summary->report_sales_persons_target : ''; ?>
                        )</p>
                    <p> Observation
                        Period: <?php echo (isset($sales_person_summary->report_observation_status) && ($sales_person_summary->report_observation_status == 1)) ? 'YES' : 'NO'; ?> </p>
                    <p> Sales
                        Code:<?php echo isset($sales_person_summary->report_ExecutiveCode) ? $sales_person_summary->report_ExecutiveCode : ''; ?> </p>
                    <br/>

                    <table style="margin-bottom: 40px; font-weight: bold">
